---
title: Friska Örnsköldsvik
---

# Friska Örnsköldsvik

<img
  class="site-logo"
  src="assets/web_arrangorer_liggande_2026.png"
  alt="Friska Örnsköldsvik och medverkande arrangörer">

Här hämtar du kartor så att du, med hjälp av telefonens GPS, kan se din position på orienteringskartan i appen QField.

## Guider

Ta del av en guide som visar hur du hämtar appen, laddar ner kartan till telefonen och importerar den i QField:

<a class="download-button"
   href="instructions/guide_qfield_ios.pdf">
  Guide för iOS (iPhone)
</a>

<a class="download-button"
   href="instructions/guide_qfield_android.pdf">
  Guide för Android (SAMSUNG m.fl.)
</a>

## Kartor
<a class="download-button"
   href="FO26_K1_Storstensudden.tif">
  Hämta karta 1 - Storstensudden
</a>
<a class="download-button"
   href="FO26_K2_Skeppsmalen.tif">
  Hämta karta 2 - Skeppsmalen
</a>
<a class="download-button"
   href="FO26_K4_Huggsjostugan.tif">
  Hämta karta 4 - Huggsjöstugan
</a>
<a class="download-button"
   href="FO26_K5_Navertjal.tif">
  Hämta karta 5 - Nävertjäl
</a>
<a class="download-button"
   href="FO26_K7_test.tif">
  Hämta karta 1 - test
</a>
## Kontakt

Ingmar Andersson 070-636 87 51
<br>
Per Sundin 073-960 16 63
<br>
Vivianne Edblom 070-631 10 28
